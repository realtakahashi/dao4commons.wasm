    ...
    <>
      <div className="bg-black flex flex-col min-h-screen">
        <div className="text-blue-300 text-center text-25px ">
          <button className="underline" onClick={() => _backToTokenList()}>
            Back To Token List
          </button>
        </div>
        <div className="p-8"></div>
        {props._selectToken.tokenKind == TokenKind.ERC20 &&
          props._forMember == true && (
            <Erc20Detail
              selectToken={props._selectToken}
              daoAddress={props.daoAddress}
            ></Erc20Detail>
          )}
        {props._selectToken.tokenKind == TokenKind.ERC20 &&
          props._forMember == false && (
        ...
        {props._selectToken.tokenKind == TokenKind.ERC721 &&
          props._forMember == true && (
            <Erc721Detail 
            daoAddress={props.daoAddress}
            selectToken={props._selectToken}></Erc721Detail>
          )}
        {props._selectToken.tokenKind == TokenKind.ERC721 &&
          props._forMember == false && (
            <Erc721ForSale selectToken={props._selectToken}></Erc721ForSale>
          )}
        {props._selectToken.tokenKind == TokenKind.GOVERNANCE &&
          props._forMember == true && (
            <GovernanceTokenDetail
              selectToken={props._selectToken}
              daoAddress={props.daoAddress}
            ></GovernanceTokenDetail>
          )}
